...
class Describe:
    ...
    def __desc(self, df: pd.DataFrame) -> pd.DataFrame:
        desc = (
            pd.DataFrame({"dtypes": df.dtypes, "nunique": df.nunique(), "nans": df.isna().sum()})
            .reset_index(level=0)
            .merge(df.describe().T.reset_index(level=0), how="left")
            .sort_values("nunique")
        )
        return desc
    ...
    def countplot(
        ...
        nuniques: int,
        cols: int = 2,
        hue: Union[str, List[int], None] = None,
        figsize: Tuple[int, int] = (10, 10),
        fontsize: int = 14,
    ) -> None:  # TODO define out of the class
...
def distplots(
    ...
    hue: Union[Any, None] = None,
    subplots_params: Union[Dict[Any, Any], None] = None,
) -> None:
    """Plot distplot.
    ...
    if isinstance(hue, str) or hue is None:
        hue = [hue] * len(columns)
    s_params = {"figsize": (15, len(columns) * 8)}
